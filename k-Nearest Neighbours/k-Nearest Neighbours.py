...
validationSet = numpy.genfromtxt("validation1.csv", delimiter=";", usecols=[0, 1, 2, 3, 4, 5, 6, 7],
                                 converters={5: lambda s: 0 if s == b"-1" else float(s),
                                             7: lambda s: 0 if s == b"-1" else float(s)})
days = numpy.genfromtxt("days.csv", delimiter=";", usecols=[0, 1, 2, 3, 4, 5, 6, 7],
                        converters={5: lambda s: 0 if s == b"-1" else float(s),
                                    7: lambda s: 0 if s == b"-1" else float(s)})
...
def GetDistance(learnSet, testSet):
    ...
    for rowId in range(len(learnSet)):  # For all elements in learnSet
        ...
        for i in range(1, learnSet.shape[1]):  # For every element(form 1) in the seconde row of the 2d array
            sum += numpy.abs(testSet[i] - learnSet[rowId][
                i]) ** 2  # Calculates the difference between the givven data point and existing data
        distance.append(
            [learnSet[rowId][0], numpy.sqrt(sum)])  # Adds the id and Pythagorean theorem result to the 2d array
    return distance
...
def GetLabels(nearest):
    ...
    for index in range(len(nearest)):  # For every element in the list that was givven
        # The modulo is used to be able to work, on data different than data form 2000
        label = nearest[index][0] % 10000
...
def Validater(learnSet, validationSet, k):
    ...
    for i in range(len(validationSet)):  # For every element in the validation set
        if MostCommonInList(GetLabels(GetNearest(GetDistance(learnSet, validationSet[i]), k))) == \
                listOfCorrectlabels[i]:  # If the guessed label is equal to the correct label
            numberOfCorrectGuesses += 1  # Then Up the count of correct guesses
    return 100 / len(validationSet) * numberOfCorrectGuesses  # Get percentage of correctness


def ValidateRange(learnSet, validationSet, beginK, stopK):
    if beginK <= 0:  # If the given K is smaller than 01
        beginK = 0  # the K will be 0
    if stopK >= len(learnSet):  # If the given K is bigger than the length of the learn set
        stopK = len(learnSet)  # The K will become the length of the learn set
    for i in range(beginK, stopK):  # For every K in between beginK and stopK
        print(f"{i}: {Validater(learnSet, validationSet, i)}")  # Print percentage of correct guesses


def GuessDays(learnSet, days, k):
    for i in range(len(days)):
        # Best guess of a givven day will be calculated and printed
        print(f"Day {i} is a {MostCommonInList(GetLabels(GetNearest(GetDistance(learnSet, days[i]), k)))} day.")


print("==============================")
GuessDays(learnSet, days, 59)  # Guesses the given days with a K of 59
print("==============================")
print(f"58: {Validater(learnSet, validationSet, 58)}")  # Calculates the correctness with a K of 58
print("==============================")
ValidateRange(learnSet, validationSet, 54, 64)  # Calculates the correctness with a K of 54 to 64
print("==============================")
